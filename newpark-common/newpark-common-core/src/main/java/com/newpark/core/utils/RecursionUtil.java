...
public class RecursionUtil {
    ...
    /**
     * 将列表递归成树（单个根节点），默认方法 "getId"，"getParentId"，"setChildren"
     */
    public static <T, K> List<T> treeByRoot(boolean isRoot, Class<T> clazz, List<T> list, K parentId) {
        ArrayList<K> parentIdList = new ArrayList<>();
     ...
     */
    private static <T, K> List<T> tree(boolean isRoot, Class<T> clazz, String getIdName, String getPidName, List<T> list, List<K> parentIdList) {
    ...
    private static <T, K> List<T> tree(boolean isRoot, Class<T> clazz, String getIdName, String getPidName, String setChildernName, List<T> list, List<K> parentIdList) {
     ...
     * 获取列表+ 树数据
     */
    public static <T, K> HashMap<String, List<T>> listAndTree(boolean isRoot, Class<T> clazz, String getIdName, List<T> list, List<K> parentIdList) {
        HashMap<String, List<T>> map = new HashMap<>(2);
        map.put("list", list);
        map.put("tree", tree(isRoot, clazz, getIdName, DEF_PID_NAME, new CopyOnWriteArrayList<>(list), parentIdList));
        return map;
     ...
     *
     * @param list           节点列表
     * @param parentNodeList 递归获取的父节点列表
     * @param clazz          节点type
     * @param getIdName      获取对象主键方法名称，反射获得
     * @param nodePid        需要递归获取父节点的某个节点pId
     * @param nodeId         需要递归获取父节点的某个节点id
     * @param <T>            遍历列表对象的泛型
     * @param <ID>           遍历列表对象主键泛型
     * @return 返回的列表是从子节点到父节点的顺序排序
     */
    public static <T, ID> List<T> listParentNode(List<T> list, List<T> parentNodeList, Class<T> clazz, String getIdName, ID nodePid,
                                                 ID nodeId) {
        try {
            Method getId = clazz.getMethod(getIdName);
            Method getParentId = clazz.getMethod(DEF_PID_NAME);
            for (T value : list) {
                // 获取对象主键方法
                ID id = (ID) getId.invoke(value);
                // 获取对象父节点方法
                ID pid = (ID) getParentId.invoke(value);
                //判断符合条件的节点
                if (id.equals(nodePid) && !id.equals(nodeId)) {
                    parentNodeList.add(value);
                    listParentNode(list, parentNodeList, clazz, getIdName, pid, id);
                    break;
                }
            }
        } catch (ReflectiveOperationException e) {
